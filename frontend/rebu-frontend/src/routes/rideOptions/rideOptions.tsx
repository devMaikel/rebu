...
export default function RideOptions() {
    ...
    const { data, rideBasics } = location.state as { data: EstimateResponse; rideBasics: RideBasicsData };
    const drivers: Driver[] = data.options;
  
    const handleChooseDriver = async (driverId: number, driverName: string, value: number) => {
            ...
            {drivers.length === 0 ? (
                ...
                {drivers.map((driver) => (
                    ...
                    <div>
                        ...
                        <p><strong>Veículo:</strong> {driver.car}</p>
                        <p><strong>Avaliação:</strong> {driver.rating} ★</p>
                        <p><strong>Valor da Viagem:</strong> R$ {driver.value}</p>
                        <button
